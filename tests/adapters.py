...
def run_embedding(
    ...
    if weights is not None:
        ...
        embedding_layer.load_state_dict(weight_state, strict=True)
...
def run_multihead_self_attention(
    ...
    """
    from cs336_basics.model.modules import MultiHeadSelfAttention
    # 1. 初始化模型（不带 RoPE）
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    from cs336_basics.model.modules import MultiHeadSelfAttention, RotaryPositionalEmbedding
    ...
    return mha_rope(in_features, token_positions=token_positions)
...
def run_transformer_block(
    ...
    # 生成位置信息 token_positions
    # 假设 in_features 形状为 (batch, seq_len, d_model)
    ...
    with torch.no_grad():
        ...
        # 注意：根据文档描述，FFN 的权重需要转置以匹配你的 Linear 模块 (out, in) 格式
...
# 精确加载权重
    with torch.no_grad():
        ...
        for i in range(num_layers):
            ...
            layer.attn.q_proj.weight.copy_(weights[f"layers.{i}.attn.q_proj.weight"])
            layer.attn.k_proj.weight.copy_(weights[f"layers.{i}.attn.k_proj.weight"])
            layer.attn.v_proj.weight.copy_(weights[f"layers.{i}.attn.v_proj.weight"])
            layer.attn.o_proj.weight.copy_(weights[f"layers.{i}.attn.output_proj.weight"])

            # FFN 子层
            layer.ffn_norm.weight.copy_(weights[f"layers.{i}.ln2.weight"])
            # 根据 Docstring 的形状描述，FFN 权重需要转置以匹配 Linear(in, out) 内部存储的 (out, in)
        ...
        # 最终归一化层 (注意键名对照 Docstring)
        model.ln_f.weight.copy_(weights["ln_final.weight"])
        
        # 输出投影层 (LM Head) (注意键名对照 Docstring)
        model.output_layer.weight.copy_(weights["lm_head.weight"])
...
def run_get_batch(
    ...
    """
    from cs336_basics.trainer.utils import get_batch
    return get_batch(
        data=dataset,
...
def run_cross_entropy(
    ...
    return cross_entropy(inputs, targets=targets)
...
def run_save_checkpoint(
    ...
    """
    from cs336_basics.trainer.check_point import save_checkpoint
    save_checkpoint(
        model=model,
        optimizer=optimizer,
        iteration=iteration,
...
def run_load_checkpoint(
    ...
    """
    from cs336_basics.trainer.check_point import load_checkpoint
